...
def predict_tile_with_targeted_fgsm(tile_path, target_class="Other"):
    ...
    for step in range(num_steps):
        ...
        # Compute the targeted loss (minimize distance to target class)
        ...
        with torch.no_grad():
            ...
            x_adv = x_adv - alpha * grad_sign  # Apply perturbation (subtract to move toward target class)
    ...
    cv2.imwrite(f"tlight-targeted-attack81-to-{target_class}-eps{epsilon}-n{num_steps}.png", 
                cv2.cvtColor(perturbed_image_np, cv2.COLOR_RGB2BGR))

# Test the function with the specified model and image path
predict_tile_with_targeted_fgsm("data/Training/Traffic Light/Tlight (81).png", target_class="Other")  # Replace with your image path
