...
def add_adversarial_patch_with_saliency(tile_path, patch_size=20, alpha=0.1):
    """
    ...
    y = torch.tensor([y_index], dtype=torch.long)  # Target class tensor
    ...
    loss = F.cross_entropy(logits, y)  # Minimize the loss to the target class
    ...
    cv2.imwrite(f"adversarial-patch-saliency-patchsize{patch_size}.png", 
                cv2.cvtColor(perturbed_image_np, cv2.COLOR_RGB2BGR))

# Test the function
add_adversarial_patch_with_saliency("data/Training/Traffic Light/Tlight (81).png", patch_size=50, alpha=0.1)
